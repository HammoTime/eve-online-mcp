...
6. For the first publication only, use the repository's configured `NPM_TOKEN` secret if npm cannot yet establish trusted publishing. Once the package exists, configure or verify the `HammoTime/eve-online-mcp` trusted publisher for `.github/workflows/release.yml`, prefer OIDC for future releases, and remove obsolete long-lived publishing credentials when the integration permits it.
